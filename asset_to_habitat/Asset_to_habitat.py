...
if __name__ == '__main__':
    ...
    try:
        ...
        parser.add_argument(
            "--glb_folder_path",
            default=project_path+"/data/assets/",
            type=str,
            help='Folder path of assets to be converted (default: "./data/assets/"), must end with \'/\''
        )
        parser.add_argument(
            "--output_folder",
            default=project_path+"/data/",
            type=str,
            help='Path for storing converted files (default: "./data/"), must end with \'/\''
        )
        parser.add_argument(
            "--dataset_name",
        ...
        if not (args.glb_folder_path).endswith('/') or not (args.output_folder).endswith('/'):
            raise PathError('args.glb_folder_path and args.output_folder should end with char \'/\'')
    except:
        print(f'Environment variable "PROJ_PATH" has not been set')
    
    
    try:
        if not os.path.exists(args.glb_folder_path):
            raise PathError('args.glb_folder_path not exist!')
        configs_static_objects_path = args.output_folder + args.dataset_name + "/configs/objects/static/"
        configs_dynamic_objects_path = args.output_folder + args.dataset_name + "/configs/objects/dynamic/"
        configs_stages_path = args.output_folder + args.dataset_name + "/configs/stages/"
        static_objects_path = args.output_folder + args.dataset_name + "/objects/static/"
        dynamic_objects_path = args.output_folder + args.dataset_name + "/objects/dynamic/"
        stages_path = args.output_folder + args.dataset_name + "/stages/"
        configs_scenes_path = args.output_folder + args.dataset_name + '/configs/scenes/'
        configs_lighting_path = args.output_folder + args.dataset_name + '/configs/lighting/'
        configs_ssd_path = args.output_folder + args.dataset_name + '/configs/ssd/'
        configs_navmeshes_path = args.output_folder + args.dataset_name + '/navmeshes/'
        urdf_path = args.output_folder + args.dataset_name + '/urdfs'
        ...
        for _id, file_name in enumerate(os.listdir(args.glb_folder_path + "/objects/static")):
            # copy .glb file to new folder
            source_file_path = os.path.join(args.glb_folder_path + "/objects/static/", file_name)
            ...
            json_file_path = configs_static_objects_path + prefix + ".object_config.json"
            data = {
                "render_asset": static_objects_path + file_name,
                "collision_asset": static_objects_path + file_name
            }
            ...
            instance = {}
            instance.update({"template_name": "objects/static/"+prefix, "motion_type": "STATIC", "translation": mesh.centroid.tolist()})
        ...
        for _id, file_name in enumerate(os.listdir(args.glb_folder_path + "/objects/dynamic")):
            # copy .glb file to new folder
            source_file_path = os.path.join(args.glb_folder_path + "/objects/dynamic/", file_name)
            ...
            # generate .object_config.json file for each object
            prefix = os.path.splitext(file_name)[0]
            json_file_path = configs_dynamic_objects_path + prefix + ".object_config.json"
            data = {
                "render_asset": dynamic_objects_path + file_name,
                "collision_asset": dynamic_objects_path + file_name
            }
            semantic_id.append({"name": prefix, "id": _id + 1})
            # generate object instance dictionary
        ...
        for file_name in os.listdir(args.glb_folder_path + "/stages"):
            # copy .glb file to new folder
            source_file_path = os.path.join(args.glb_folder_path + "/stages/", file_name)
            destination_file_path = os.path.join(stages_path, file_name)
            shutil.copyfile(source_file_path, destination_file_path)
            # generate .object_config.json file for each object
            prefix = os.path.splitext(file_name)[0]
            json_file_path = configs_stages_path + prefix + ".stage_config.json"
            data = {
                "render_asset": stages_path + file_name,
                "collision_asset": stages_path + file_name
            }
            ...
            if "template_name" not in stage_instance:
                ...
                bounds = mesh.bounds
                stage_instance.update({"template_name": "stages/" + prefix, "translation": mesh.centroid.tolist()})
                scene_prefix = prefix
        ...
        # for f in os.scandir(args.glb_folder_path + 'urdfs/'):
        ...
        with open(configs_lighting_path + scene_prefix + '.lighting_config.json', 'w') as light_json:
            json.dump(lighting_instance, light_json, indent=4)
        light_json.close()

        # generate semantic id
        with open(configs_ssd_path + args.scene_name + '_semantic_lexicon.json', 'w') as semantic_json:
        ...
        cal_navmesh(configs_scenes_path + args.scene_name + '.scene_instance.json', \
                    args.output_folder + args.dataset_name + '/' + scene_prefix + '.scene_dataset_config.json', \
                    configs_navmeshes_path + args.scene_name + '.navmesh')
